...
describe('admin manage', () => {
  ...
  it('生成卡 + 列卡', async () => {
    ...
    const gen = await app.inject({ method: 'POST', url: '/admin/cards/generate', headers: A(t), payload: { count: 2, kind: 'point', points: 100 } })
  ...
  it('调积分写流水', async () => {
    const t = await adminToken()
    await registerWithPoints(app, 'u1', 100)   // 点卡注册给 100 底分
